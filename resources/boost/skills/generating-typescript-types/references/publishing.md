...
| A model interface is missing all its columns          | The model's table is not migrated on its connection. For a model with no accessors either, `ts:publish` says so after its summary (`Table [...] does not exist on connection [...]`) and still exits 0. Run the migrations there; `php artisan migrate` republishes with `--fresh` for you |
...
| `TS6196` / `TS6192` in a generated file               | Only under `noUnusedLocals`: a resource's `#[TsCasts]` value that spells an imported type's name inside a string, template literal or comment (`'User' \| 'Admin'`) keeps that import. Respell the literal, or override with a type that uses the import; do not edit the generated file                                     |
