...
- `config('literal.key')` types from the live config value; `$request->url()/path()/integer()/boolean()/string()/cookie()/hasCookie()`
  type from Laravel's signatures; `Inertia::defer()/optional()/lazy()` make the key optional; `always()/merge()/deepMerge()`
...
| `EnumResource::make($x)`                                                       | `AsEnum<typeof Enum>`                                           |
| an enum case or enum-typed value                                               | `{Enum}Type`                                                    |
...
- A value from a service/repository call, a closure, or a variable assigned in a branch.
...
- Declaring a `SharedData` type by hand in `resources/js/types/index.d.ts`, which collides with the generated global.
