...
| A `Collection` member's `->only([...])` / `->except([...])`: a `'collection'` or `AsCollection` column, or an accessor or method returning an `Illuminate\Support\Collection` | `Record<string, unknown>` for any key list and any element type, because a collection keeps the entries whose keys are listed. `?->` adds `\| null`. An accessor returning an Eloquent collection publishes its own list (`Comment[]`), as a to-many relation does |
