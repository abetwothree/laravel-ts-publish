...
| A `Collection` member's `->only([...])` / `->except([...])`: an `AsCollection` column, or an accessor or method returning one                                                                     | `Record<string, unknown>` for any key list, because a collection keeps the entries whose keys are listed. `?->` adds `\| null`. A to-many relation is an Eloquent collection and keeps its row above                                                                                                                                                                                     |
