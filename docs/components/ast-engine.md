...
contract every handler implements — it is what lets 26 independently-written handlers reproduce one
...
`ArrayMergeHandler`, `InertiaWrapperHandler`, `ReceiverPropertyFetchHandler`, and
`ReceiverMethodCallHandler`. `ReceiverMethodCallHandler`
