...
to-many relation's filter, a map proxy, a multi-model accessor's filter, or a filter on a column cast with
`AsCollection`, which publishes `Record<string, unknown>`. A single relation's filter, and one on an accessor
returning a `Support\Collection`, also reach `ReceiverMethodCallHandler`, which gives the same answer. `Comment::relationPicks()` pins the handler through its
