...
- **Relation `only()` / `except()`** — `$this->relation->only([...])` / `->except([...])` references the related model's generated interface as `Pick<Model, 'a' | 'b'>` (`except()` picks the complement, every other column), keeping its `#[TsCasts]` and `@property` refinements, whenever the relation resolves to a single model **and** every filtered key is a real database column. Anything else expands inline, where `except()` yields **database columns only**, and a runtime key list publishes `Record<string, unknown>`. On a to-many relation, `only()` and `except()` filter the loaded models by primary key, so `$this->comments->only(...)` publishes the relation's own type, `Comment[]`. A member whose collection class runs `Illuminate\Support\Collection`'s own `only()`/`except()` — a `'collection'`, `'encrypted:collection'`, `AsCollection` or `AsEncryptedCollection` column, or an accessor, cast getter or method returning a `Collection` — keeps the entries whose keys are listed, so its filter publishes `Record<string, unknown>` whatever the collection holds. Two such members stay `unknown`: an `AsEnumCollection` column, and one of those cast columns read through another model (`$this->twin->options->only([...])`). A collection class that overrides `only()`/`except()` publishes `unknown` too — a `using()` class or a returned subclass that overrides them, a cast that builds an Eloquent collection, and a method returning an Eloquent collection — with one exception: an accessor returning an Eloquent collection of models filters them by primary key like a to-many relation and returns them as a list, so its filter publishes a list of those models, such as `Comment[]`, whatever key type the accessor declares. The `$this->resource->relation` spelling publishes the same types — see [Relation Filters](https://tolki.abe.dev/ts/api-resources.html#relation-filters).
