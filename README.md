...
Instance methods (decorated with `#[TsEnumMethod]`) are flattened as top-level keys with the resolved value **for the specific case** passed to the resource. Static methods (decorated with `#[TsEnumStaticMethod]`) are also flattened as top-level keys with their return values.
...
- Only methods marked with `#[TsEnumMethod]` or `#[TsEnumStaticMethod]` (or all public methods when auto-include is enabled) are included.
