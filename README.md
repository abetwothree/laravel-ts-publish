...
- **Collection pipelines and helpers** — a trailing `->all()`, a `concat()` of the same collection type, `collect(explode(…))->map()`, `data_get()`, `->only([...])` in value position, and `new SomeResource(…)->resolve()` all carry their element type to the end of the chain. `only()` and `except()` reference the model's own interface as `Pick<Model, …>` only when the call carries a literal key list, so `$this->only($request->input('fields'))` publishes `Record<string, unknown>`.
...
- **Every return branch counts** — a `return []` guard makes the keys the other branches publish optional, a spread method's own `@return array{…}` fills the keys its body left `unknown`, and a key built from literal text around a variable (`$data["{$name}_label"]`) publishes as a template-literal index signature. The body always wins over the docblock, so a stale `@return` can never overwrite a type the analyzer already resolved.
