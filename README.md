...
- **Collection pipelines and helpers** — a trailing `->all()`, a `concat()` of the same collection type, `collect(explode(…))->map()`, `data_get()`, `->only([...])` in value position, and `new SomeResource(…)->resolve()` all carry their element type to the end of the chain. `only()` and `except()` reference the model's own interface as `Pick<Model, …>` only when the call carries a literal key list, so `$this->only($request->input('fields'))` keeps the vague shape.
