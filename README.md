...
- **Local variables, narrowing, and closures** — a value read through a local variable, a closure-local, an `if (! $x instanceof Post) { return null; }` guard, or an `instanceof` ternary keeps its type instead of degrading to `unknown`. The guard binds only a variable the method writes once, and only for the reads after it. When a ternary's true arm is the expression its `instanceof` test (or `||` chain of tests) checks, reads through a variable assigned from it narrow too: after `$v = $this->imageable instanceof Post || $this->imageable instanceof User ? $this->imageable : null;`, `$v?->getKey()` publishes `number | null` instead of the morph union's `number | string | null`, though `$v` itself still publishes the whole union. A typed `map()` parameter types a chained read too, so `$comment->user?->name` inside `$rows->map(fn (Comment $comment) => …)` publishes `string | null`.
